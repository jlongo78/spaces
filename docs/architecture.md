...
Spaces is a workspace manager for AI coding agents. It runs as two cooperating processes:
...
> **Note:** `IS_DESKTOP` is a misnomer. It really means "personal/local mode". Renaming it would touch many files, so it's left as-is with this caveat.
...
- **sessions** — Agent session metadata (imported from `~/.claude/`, `~/.codex/`, `~/.gemini/`, and project-local `.aider.chat.history.md` files)
