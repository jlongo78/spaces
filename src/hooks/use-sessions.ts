...
export function useAddSessionToWorkspace() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ workspaceId, sessionId, nodeId, title, cwd }: {
      workspaceId: number; sessionId: string; nodeId?: string; title?: string; cwd?: string;
    }) => {
      const res = await fetch(api(`/api/workspaces/${workspaceId}/sessions`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sessionId, nodeId, title, cwd }),
      });
