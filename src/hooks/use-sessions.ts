...
interface SessionsParams {
  ...
  offset?: number;
  limit?: number;
  nodeFilter?: 'local' | 'all' | string;
}

interface SessionsResponse {
  sessions: SessionWithMeta[];
  total: number;
  errors?: { nodeId: string; nodeName: string; error: string }[];
}
...
export function useSessions(params: SessionsParams = {}) {
  return useQuery({
    ...
    queryFn: async (): Promise<SessionsResponse> => {
      ...
      if (params.nodeFilter === 'all') sp.set('nodes', 'all');
...
export function useMessages(sessionId: string | null, offset = 0, limit = 50, nodeId?: string) {
  return useQuery({
    queryKey: ['messages', sessionId, offset, limit, nodeId],
    queryFn: async (): Promise<{ messages: ParsedMessage[]; total: number; hasMore: boolean }> => {
      const url = nodeId
        ? api(`/api/network/proxy/${nodeId}/sessions/${sessionId}/messages?offset=${offset}&limit=${limit}`)
        : api(`/api/sessions/${sessionId}/messages?offset=${offset}&limit=${limit}`);
...
export function useRemoteWorkspaces() {
  return useQuery({
    ...
    queryFn: async (): Promise<{
      local: Workspace[];
      remote: { nodeId: string; nodeName: string; workspaces: Workspace[] }[];
      errors: { nodeId: string; nodeName: string; error: string }[];
    }> => {
      const res = await fetch(api('/api/workspaces?nodes=all'));
      return res.json();
    },
  });
}
...
export function useAddSessionToWorkspace() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ workspaceId, sessionId, nodeId, title, cwd, agentType }: {
      workspaceId: number; sessionId: string; nodeId?: string; title?: string; cwd?: string; agentType?: string;
    }) => {
      const res = await fetch(api(`/api/workspaces/${workspaceId}/sessions`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sessionId, nodeId, title, cwd, agentType }),
      });
...
export function useSearch(query: string, projectId?: string, nodes?: 'all' | 'local') {
  return useQuery({
    queryKey: ['search', query, projectId, nodes],
    queryFn: async (): Promise<{ results: SearchResult[]; query: string; errors?: any[] }> => {
      const sp = new URLSearchParams({ q: query });
      if (projectId) sp.set('projectId', projectId);
      if (nodes === 'all') sp.set('nodes', 'all');
