...
import {
  ...
  Pencil, Layers, Check, X, TagIcon, Globe,
} from 'lucide-react';
...
function SessionRow({ session, isSelected, onToggle }: {
  ...
  return (
    <div className={cn(
      ...
      {/* Star — disabled for remote sessions */}
      {!session.nodeId ? (
        <button
          onClick={(e) => {
            e.preventDefault();
      ...
      {/* Content */}
      <Link href={session.nodeId ? `/sessions/${session.id}?node=${session.nodeId}` : `/sessions/${session.id}`} className="flex-1 min-w-0">
...
function ActionMenu({ session, onClose }: { session: SessionWithMeta; onClose: () => void }) {
  ...
  return (
    ...
    >
      {/* Rename — local sessions only */}
      {!session.nodeId && (renaming ? (
      ...
      {/* Star/Unstar — local sessions only */}
      {!session.nodeId && (
        <button
          onClick={() => {
            toggleStar.mutate(session.id);
      ...
      {/* Add tag — local sessions only */}
      {!session.nodeId && (tagging ? (
        <div className="px-3 py-2">
          <input
            autoFocus
            value={tagValue}
            ...
            <button
              ...
              onClick={() => {
                addToWorkspace.mutate({
                  ...
                  title: session.customName || session.summary || session.firstPrompt?.slice(0, 50) || session.projectName,
