...
export function TotpGate({ children }: TotpGateProps) {
  ...
  useEffect(() => {
    ...
    // In server mode with self-contained auth, check if the user has
    // an active session. The httpOnly session cookie can't be read via
    // document.cookie, so we call /api/auth/me instead.
    fetch(api('/api/auth/me'))
      .then(r => {
        if (r.ok) {
          // User has valid session — already authenticated with TOTP at login
          setToken('session-auth');
          setStatus('authorized');
          return;
        }
        // No session — fall through to legacy TOTP flow
        return checkLegacyTotp();
      })
      .catch(() => checkLegacyTotp());

    function checkLegacyTotp() {
  ...
  const verify = useCallback(async (verifyCode: string) => {
    ...
    if (data.success && data.terminalToken) {
      sessionStorage.setItem(SESSION_KEY, data.terminalToken);
      setToken(data.terminalToken);
      setStatus('authorized');
    } else {
