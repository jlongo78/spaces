...
export default function SessionViewerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    fetch(api(`/api/network/proxy/${nodeId}/sessions/${id}`))
      .then(r => r.json())
      .then(data => { setRemoteSession(data); setRemoteLoading(false); })
      .catch(() => setRemoteLoading(false));
  }, [id, nodeId, isRemote]);

  // ─── Data ──────────────────────────────────────────────
  const { data: localSession, isLoading: localSessionLoading } = useSession(isRemote ? null : id);
  const session = isRemote ? remoteSession : localSession;
  const sessionLoading = isRemote ? remoteLoading : localSessionLoading;

  const { data: messagesData, isLoading: messagesLoading } = useMessages(id, loadedPages * limit, limit, nodeId);
  ...
  const refreshMessages = useCallback(() => {
    ...
    }, 1500); // Wait for agent to finish writing to JSONL
  }, [id, queryClient]);
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-4 py-2.5 flex-shrink-0 z-10">
          ...
          {/* Tags + Workspaces bar (local sessions only) */}
          {!isRemote && (
        ...
        {isRemote && (
          <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-blue-950/30 border-b border-blue-200 dark:border-blue-800 text-sm text-blue-700 dark:text-blue-300">
            <Globe className="w-4 h-4" />
            <span>Remote session — viewing from a connected node</span>
          </div>
        )}

        {/* Notes panel (local only) */}
        {notesOpen && !isRemote && (
