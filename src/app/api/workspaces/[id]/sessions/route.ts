...
export async function POST(
  ...
  return withUser(user, async () => {
    ...
    const title = session
      ? (session.customName || session.summary || session.firstPrompt?.slice(0, 50) || session.projectName || 'Session')
      : (body.title || 'Remote Session');
    const cwd = session ? (session.projectPath || '/tmp') : (body.cwd || '/tmp');
    const agentType = session?.agentType || body.agentType || 'claude';
    const color = AGENT_TYPES[agentType]?.color || '#8b5cf6';
