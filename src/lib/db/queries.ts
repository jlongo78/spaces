...
export function upsertProject(project: { id: string; name: string; path: string; claudePath: string }) {
  const db = getDb();
  db.prepare(`
    INSERT INTO projects (id, name, path, claude_path)
    VALUES (?, ?, ?, ?)
    ON CONFLICT(id) DO UPDATE SET name=excluded.name, path=excluded.path, claude_path=excluded.claude_path
  `).run(project.id, project.name, project.path, project.claudePath);
}
...
export function upsertSession(session: {
  ...
  const db = getDb();
  db.prepare(`
    INSERT INTO sessions (id, session_id, project_id, first_prompt, summary, message_count, created, modified, git_branch, project_path, full_path)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  `).run(
    ...
    session.projectPath, session.fullPath
  );
}
...
export function createPane(pane: {
  ...
  db.prepare(`
    INSERT INTO panes (id, title, color, cwd, claude_session_id, agent_type, custom_command, sort_order, workspace_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(pane.id, pane.title, pane.color, pane.cwd, pane.claudeSessionId || null, pane.agentType || 'shell', pane.customCommand || null, order, wsId || null);
