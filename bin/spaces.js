...
if (!fs.existsSync(standaloneServer)) {
  ...
  console.error('  Run "npm run build:npm" first, or install via "npm install -g agent-spaces".');
  process.exit(1);
}
...
const termServer = spawn(process.execPath, [
  path.join(__dirname, 'terminal-server.js'),
], {
  cwd: projectDir,
  env: { ...process.env, SPACES_WS_PORT: String(WS_PORT), SPACES_TIER: process.env.SPACES_TIER || 'federation' },
  stdio: 'pipe',
});
