...
const SPACES_TIER = process.env.SPACES_TIER || 'federation';
...
wss.on('connection', (ws, req) => {
  ...
  if (sessionPayload) {
    ...
    // Desktop/dev mode: accept 'desktop-local' or 'session-auth' tokens directly
    if (terminalToken === 'desktop-local' || terminalToken === 'session-auth') {
      ...
      if (tokenUser) {
        // If SSO header is present, prefer that for username (reverse proxy setups)
        const ssoUser = req.headers['x-auth-user'];
        username = ssoUser || tokenUser;
  ...
  if (!username) {
    ...
    if (apiKey) {
      console.log(`[Auth] API key provided: ${apiKey.slice(0, 12)}... (length=${apiKey.length})`);
...
async function handleProxyConnection(clientWs, nodeId, opts) {
  ...
  // Federation targets are user-trusted nodes; skip TLS verification for self-signed certs
  ...
  if (customCommand) remoteParams.set('customCommand', customCommand);
  ...
  console.log(`[Proxy] wsUrl=${wsUrl} apiKey=${apiKey.slice(0, 12)}...`);
  console.log(`[Proxy] Connecting to remote: ${remoteUrl.replace(apiKey, '***')}`);

  const remoteWs = new WebSocket(remoteUrl, { rejectUnauthorized: false });
...
// Start mDNS discovery (federation tier only)
if (SPACES_TIER === 'federation') {
  try {
